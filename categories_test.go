...
// TestIncludeCategory validates include/exclude logic.
func TestIncludeCategory(t *testing.T) {
	includeSet := listToSet([]string{"node-modules", "python-venv"})
	excludeSet := listToSet([]string{"__pycache__"})

	// In include set, not in exclude set
	if !includeCategory("node-modules", includeSet, nil) {
		t.Fatalf("expected node-modules to be included")
	}

	// In exclude set
	if includeCategory("__pycache__", nil, excludeSet) {
		t.Fatalf("expected __pycache__ to be excluded")
	}

	// Not in include set (include set is active)
	if includeCategory("rust-target", includeSet, nil) {
		t.Fatalf("expected rust-target to be excluded by include set")
	}

	// No sets active — everything passes
	if !includeCategory("anything", nil, nil) {
		t.Fatalf("expected anything to pass with no sets")
	}

	// Exclude overrides include
	if includeCategory("node-modules", includeSet, listToSet([]string{"node-modules"})) {
...
func TestNewDirCategory(t *testing.T) {
	...
	if cat.Display != "Test Dir" {
		t.Fatalf("expected display 'Test Dir', got %q", cat.Display)
	}
...
// TestMatchFileCaseInsensitive validates case-insensitive extension matching.
