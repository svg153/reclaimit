...
func anonymizeReport(report scanner.Report) scanner.Report {
	...
	report.InactiveProjects = append([]scanner.InactiveProject(nil), report.InactiveProjects...)
	for i := range report.InactiveProjects {
		report.InactiveProjects[i].Path = "<redacted>"
	}
	return report
}
...
func renderPlain(report scanner.Report) string {
	...
	if len(report.InactiveProjects) > 0 {
		b.WriteString("\nReview-only inactive projects\n")
		for _, project := range report.InactiveProjects {
			fmt.Fprintf(&b, "  %s  %s across %d generated artifacts; last activity %s\n", escapePlain(project.Path), humanizeBytes(project.GeneratedBytes), project.GeneratedCount, humanizeTimestamp(project.LastActivity))
			fmt.Fprintf(&b, "            review only: %s\n", escapePlain(project.Reason))
		}
	}
	return b.String()
}
...
func renderMarkdown(report scanner.Report) string {
	...
	if len(report.CleanIssues) > 0 {
		...
		for _, issue := range report.CleanIssues {
			recovery := "-"
			if issue.QuarantinePath != "" {
				recovery = "`" + escapeMarkdownCell(issue.QuarantinePath) + "`"
			}
			fmt.Fprintf(&b, "| %s | `%s` | %s | %s |\n",
				escapeMarkdownCell(issue.Status),
				escapeMarkdownCell(issue.Path),
				escapeMarkdownCell(issue.Reason),
				recovery,
			)
		}
	}
	if len(report.InactiveProjects) > 0 {
		b.WriteString("\n## Review-only inactive projects\n\n| Project | Generated bytes | Artifacts | Last relevant activity | Review only | Reason |\n| --- | ---: | ---: | --- | --- | --- |\n")
		for _, project := range report.InactiveProjects {
			fmt.Fprintf(&b, "| `%s` | %s | %d | %s | %t | %s |\n", escapeMarkdownCell(project.Path), humanizeBytes(project.GeneratedBytes), project.GeneratedCount, escapeMarkdownCell(humanizeTimestamp(project.LastActivity)), project.ReviewOnly, escapeMarkdownCell(project.Reason))
		}
	}
