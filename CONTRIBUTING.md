...
- **Coverage**: `scripts/check-coverage.sh` enforces **90% overall and per
  production package and source file**. The process signal/bootstrap entrypoint
  and test-support packages are explicitly not applicable; command and
  cancellation behavior is tested in the root package. Do not add an exclusion
  or lower the floor to make a change pass.
